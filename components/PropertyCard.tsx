...
const PropertyCard = ({ property }: PropertyCard) => {
  // "rates": {
  //     "nightly": 200,
  //     "weekly": 750,
  //     "monthly": 3300
  ...
  return (
    <div className='rounded-xl shadow-md relative'>
      <Image src={`/properties/${property.images[0]}`} width='0' height='0' sizes='100vw' alt='' className='w-full h-auto rounded-t-xl' />
      <div className='p-4'>
        <div className='text-left md:text-center lg:text-left mb-6'>
          <div className='text-gray-600'>{property.type}</div>
          <h3 className='text-xl font-bold'>{property.name}</h3>
        </div>
        <h3 className='absolute top-[10px] right-[10px] bg-white px-4 py-2 rounded-lg text-blue-500 font-bold text-right md:text-center lg:text-right'>{getRateDisplay()}</h3>
        ...
        <div className='flex justify-center gap-4 text-gray-500 mb-4'>
          ...
          <p>
            <FaRulerCombined className='md:hidden lg:inline' /> {property.square_feet} <span className='md:hidden lg:inline'> sqft</span>
          </p>
        </div>
