...
const PropertyDetails = ({ property: propertyDoc }: { property: PropertyType }) => {
      ...
      <div className='bg-white p-6 rounded-lg shadow-md text-center md:text-left'>
        ...
        <div className='flex flex-col md:flex-row justify-around'>
          <div className='flex items-center justify-center mb-4 border-b border-gray-200 md:border-b-0 pb-4 md:pb-0'>
            ...
            <div className='text-2xl font-bold text-blue-500'>{property.rates.nightly ? `$${property.rates.nightly.toLocaleString()}` : <FaTimes className='text-red-700' />}</div>
          </div>
          <div className='flex items-center justify-center mb-4 border-b border-gray-200 md:border-b-0 pb-4 md:pb-0'>
            <div className='text-gray-500 mr-2 font-bold'>Weekly</div>
            <div className='text-2xl font-bold text-blue-500'>{property.rates.weekly ? `$${property.rates.weekly.toLocaleString()}` : <FaTimes className='text-red-700' />}</div>
          </div>
          <div className='flex items-center justify-center mb-4 pb-4 md:pb-0'>
            <div className='text-gray-500 mr-2 font-bold'>Monthly</div>
            <div className='text-2xl font-bold text-blue-500'>{property.rates.monthly ? `$${property.rates.monthly.toLocaleString()}` : <FaTimes className='text-red-700' />}</div>
          </div>
        </div>
      </div>
      ...
      <div className='bg-white p-6 rounded-lg shadow-md mt-6'>
        ...
        <ul className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 list-none'>
          {property.amenities.map((item, index) => {
            return (
              <li key={index}>
                <FaCheck className='inline-block text-green-600 mr-2' /> {item}
              </li>
            );
          })}
        </ul>
